...
const MODES = [{
  ...
  // the metropolitan area — 41.18–41.78 N, 1.63–2.50 E, 72 × 67 km — as
  // 3 × 3 tiles cut out of the Geofabrik cataluna extract (pbf-tiles.py);
  ...
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['V15']),
  feeds: [
    { tag: 'bcn', dir: 'data/gtfs', routeTypes: ['3'],
      skipRoute: (r) => !S_KEEP.has(r.route_id), mapKey: (sn, r) => lineKey(sn, r), op: true },
  ],
}];
// The rail slot splits into four cfgs sharing mode 'tram': the trams, the
// metro, the funiculars and the trains must NOT share a graph — a metro shape
// often passes closer to the tram tracks above its tunnel than to the subway
// axis, and a mixed graph then lures Viterbi onto disconnected rails.
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
const isTramLine = (l) => /^T[1-6]$/.test(l);
const isMetroLine = (l) => /^L\d/.test(l);
const isFuniLine = (l) => /^(FM|FV)$/.test(l);
const isTrainLine = (l) => /^(S\d|R\d|RL|RG|T2$)/.test(l);
const isTrunkLine = (l) => isMetroLine(l) || isTrainLine(l) || isFuniLine(l);
const tramSel = tramLines.filter(isTramLine);
const metroSel = tramLines.filter(isMetroLine);
const funiSel = tramLines.filter(isFuniLine);
const trainSel = tramLines.filter(isTrainLine);

// what the scope pass decided: which of the feed's 1 615 Catalan routes are on
// this sheet, plus the line key and operator of each
...
const S_KEEP = new Set(SCOPE.keep);
const KEY = SCOPE.key || {}, OP = SCOPE.op || {};

// With 31 operators on the sheet the same number is used many times over —
// Barcelona's 1 is also a Sabadell line and a Terrassa line — so those keys
// carry the operator's code and print bare through LBL (the Randstad rule).
// The panel groups its chips by the same code.
const LBL = new Map();
...
if (tramAll || tramSel.length) MODES.push({
  // T1, T3, T4, T5, T6 — the Trambaix and the Trambesòs. Red, like every tram
  // on these maps: the colour says the mode (the operator's own T-colours are
  // in the feed and are kept out on purpose).
  mode: 'tram', label: 'trams', osmFile: 'data/osm/barcelona-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'bcn', dir: 'data/gtfs', routeTypes: ['0'],
      skipRoute: (r) => !S_KEEP.has(r.route_id), mapKey: (sn, r) => lineKey(sn, r), op: true },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  // TMB's L1–L5 and L9…L12 plus FGC's L6, L7, L8 — all of them in the
  // operators' own colours, which the feed ships (the family's exception to
  // the mode-colour rule: a metro keeps its livery).
  mode: 'tram', label: 'metro', osmFile: 'data/osm/barcelona-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    { tag: 'bcn', dir: 'data/gtfs', routeTypes: ['1'],
      skipRoute: (r) => !S_KEEP.has(r.route_id), mapKey: (sn, r) => lineKey(sn, r), op: true },
  ],
});
if (tramAll || funiSel.length) MODES.push({
  // The two funiculars inside the city: Montjuïc (FM) and Vallvidrera (FV).
  // OSM tags them railway=funicular, which graph.mjs does not know, so they
  // are admitted by railExtra and renamed to `rail` before the graph is built
  // — the Vienna rule for a way whose tag is not in RAIL_OK.
  mode: 'tram', label: 'funiculars', osmFile: 'data/osm/barcelona-rail.json',
  graphMode: 'tram', railKeep: new Set(['rail']),
  railExtra: (e) => e.tags?.railway === 'funicular',
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : funiSel,
  feeds: [
    { tag: 'bcn', dir: 'data/gtfs', routeTypes: ['7'],
      skipRoute: (r) => !S_KEEP.has(r.route_id), mapKey: (sn, r) => lineKey(sn, r), op: true },
  ],
});
if (tramAll || trainSel.length) MODES.push({
  // The commuter railways: FGC's S1…S9 and R5…R63 out of Plaça d'Espanya and
  // Plaça de Catalunya, and Rodalies' metropolitan R1, R2, R2N, R2S, R7 and
  // R8. Drawn whole where they leave the metropolitan area — R6 keeps its run
  // to Manresa and R1 its run to Maçanet — the way Berlin draws its RB/RE.
  // The long-distance Rodalies (R13…R17, RG1) and the high-speed lines are
  // not on this sheet; pipeline/scope.mjs says why.
  mode: 'tram', label: 'trains', osmFile: 'data/osm/barcelona-rail.json',
  ...
  feeds: [
    { tag: 'bcn', dir: 'data/gtfs', routeTypes: ['2'],
      skipRoute: (r) => !S_KEEP.has(r.route_id), mapKey: (sn, r) => lineKey(sn, r), op: true },
  ],
});
...
async function processMode(cfg) {
  ...
  if (cfg.railKeep) {
    ...
    for (const e of osm.elements) {
      ...
      } else if (rw === 'narrow_gauge' || rw === 'funicular') {
        // a funicular is a railway with a scheduled service on it; graph.mjs
        // knows subway/tram/light_rail/rail and nothing else, so Montjuïc and
        // Vallvidrera are renamed before the graph is built
